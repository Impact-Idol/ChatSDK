...
export function useFileUpload(options) {
    ...
    const upload = useCallback(async (file) => {
        ...
        try {
            ...
            const presigned = await client.fetch('/api/uploads/presigned', {
                method: 'POST',
                body: JSON.stringify({
                    channelId,
                    filename: file.name,
            ...
            // Confirm upload
            const result = await client.fetch(`/api/uploads/${encodeURIComponent(presigned.key)}/confirm`, { method: 'POST' });
            const uploadedFile = {
                id: result.id,
                key: result.key,
