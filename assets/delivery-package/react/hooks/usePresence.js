...
export function usePresence(options = {}) {
    ...
    const getMultiplePresence = useCallback(async (userIds) => {
        try {
            const response = await client.fetch('/api/presence/query', {
                method: 'POST',
    ...
    // Get channel presence
    const getChannelPresence = useCallback(async (channelId) => {
        try {
            return await client.fetch(`/api/channels/${channelId}/presence`);
        }
        catch {
...
export function useChannelPresence(channelId) {
    ...
    const refresh = useCallback(async () => {
        if (!channelId)
            return;
        try {
            const response = await client.fetch(`/api/channels/${channelId}/presence`);
