 ...
 * ```
 */
export function useReadReceipts(channelId, options = {}) {
    ...
    useEffect(() => {
        if (!channelId)
            return;
        const unsub = client.on('read_receipt', (data) => {
    ...
    const getReceipts = useCallback(async (messageId) => {
        if (!channelId) {
            return { readBy: [], readCount: 0, totalMembers: 0 };
        }
        const response = await client.fetch(`/api/channels/${channelId}/receipts/messages/${messageId}/receipts`);
        // Cache the result
        setReceipts((prev) => ({
            ...prev,
            [messageId]: {
    ...
    const getMultipleReceipts = useCallback(async (messageIds) => {
        if (!channelId || messageIds.length === 0) {
            return {};
        }
        const response = await client.fetch(`/api/channels/${channelId}/receipts/query`, {
            method: 'POST',
    ...
    const getChannelReadStatus = useCallback(async () => {
        if (!channelId)
            return [];
        const response = await client.fetch(`/api/channels/${channelId}/receipts/read-status`);
 ...
 */
export function formatReadReceipt(readCount, totalMembers, isSentByCurrentUser) {
...
export function getReadReceiptStatus(messageStatus, readCount, isSentByCurrentUser) {
    if (!isSentByCurrentUser)
        return 'sent';
