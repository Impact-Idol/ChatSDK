...
async function main() {
  const tokenUrl = process.env.CHATSDK_TOKEN_URL || 'http://localhost:3000/api/chatsdk-token';

  const tokenProvider = async () => {
    const response = await fetch(tokenUrl, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ userId: 'demo-user', displayName: 'Demo User' }),
    });

    if (!response.ok) {
      throw new Error('Failed to get ChatSDK token. Configure CHATSDK_TOKEN_URL to your backend token route.');
    }

    return response.json();
  };

  // Connect to ChatSDK with user-scoped tokens from your backend
  ...
  const message = await client.sendMessage({
    ...
    text: 'Hello from ChatSDK! 🚀',
  });

  console.log('Sent message:', message.text);
  console.log('\\n🎉 Success! ChatSDK is working.');
