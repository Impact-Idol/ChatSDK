...
import type { User } from '@chatsdk/core';
 ...
 * ```
 */
export function useTypingIndicator(channelId: string | null): UseTypingIndicatorResult {
  ...
  useEffect(() => {
    ...
    const unsubStart = client.on('typing.start', ({ channelId: cid, user }) => {
      if (cid !== channelId || user.id === client.user?.id) return;
 ...
 */
export function formatTypingText(users: User[]): string {
