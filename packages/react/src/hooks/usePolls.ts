 ...
 * ```
 */
export function usePolls(messageId: string): UsePollsResult {
  ...
  const fetchPoll = useCallback(async () => {
    ...
    try {
      ...
      const message = await client.fetch(`/api/messages/${messageId}`) as any;
      ...
      // Fetch poll results
      const response = await client.fetch(`/api/polls/${message.poll_id}/results`) as any;
