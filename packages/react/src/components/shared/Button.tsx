...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      className={clsx(
        'chatsdk-button',
        `chatsdk-button-${variant}`,
        `chatsdk-button-${size}`,
        loading && 'chatsdk-button-loading',
        className
      )}
    ...
    >
      {loading && (
        <svg className="chatsdk-button-spinner" viewBox="0 0 24 24">
          <circle cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="3" fill="none" strokeDasharray="32" strokeLinecap="round" />
        </svg>
      )}
      <span className={loading ? 'chatsdk-button-content-hidden' : ''}>{children}</span>
      <style>{`
        .chatsdk-button {
          display: inline-flex;
          align-items: center;
